  ...
  <section class="container">
    ...
    <div class="questions-wrap">
      <div class="questions-list" v-if="questionList.length > 0">
        <nuxt-link v-for="item in questionList" :key="item.id" :to="`/question/${subjectText}/${item.suffix}_${item.id}`">
...
export default {
  ...
  computed: {
    subjectText() {
      const subjectItem = this.subjects.find((item) => item.value === this.searchForm.subjectId)
      return subjectItem.split(' ').join('_')
    }
  },
  created() {
    const subjectText = this.$route.params.subject.split('_').join(' ') || 'Maths'
